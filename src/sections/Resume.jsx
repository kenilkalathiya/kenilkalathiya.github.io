...
import { FaUserGraduate, FaLaptopCode, FaPalette, FaCertificate } from "react-icons/fa";
...
// `short` is the label used in the tight mobile tab row, where a full
// "Certifications" wouldn't fit one of five equal-width columns.
const TABS = [
  { name: "Education", icon: FaUserGraduate, component: Education },
  { name: "Experience", icon: MdWorkOutline, component: Experience },
  { name: "Certifications", short: "Certs", icon: FaCertificate, component: Certifications },
    ...
    <section id="resume" className="bg-night py-16 md:py-24">
      <Container>
        ...
        <RevealOnScroll className="mx-auto flex max-w-5xl flex-col overflow-hidden rounded-lg border border-hairline shadow-panel md:flex-row">
          {/* All five tabs stay visible at once on mobile (equal-width row) so
              there's never a hidden option behind a scroll the user has to
              discover; desktop keeps the vertical icon+label sidebar. */}
          <div className="grid shrink-0 grid-cols-5 border-b border-hairline bg-panel md:flex md:w-64 md:flex-col md:border-b-0 md:border-r">
            {TABS.map((tab, index) => {
              const Icon = tab.icon;
